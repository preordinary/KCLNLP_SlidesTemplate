"""Build the Royal Blue demo deck.

Run: python templates/royal_blue/build.py

A formal white-background deck with royal-blue rules and accents.
Slide titles live in the header (left, as "Now: <title>"); body
gets the full slide beneath.
"""
...
OUTPUT = Path(__file__).resolve().parent / "KCLNLP_RoyalBlue.pptx"
...
def _trimmed_logo(path: Path) -> Path:
    """Cache a copy of the logo cropped to its visible (alpha) bbox."""
...
def add_header(slide, title: str | None = None):
    ...
    if title:
        ...
        now_run.font.color.rgb = rgb(T.BLUE)
...
def slide_title(prs):
    ...
    _add_text(slide, left + Inches(0.35), title_top,
              ...
              font=T.FONT_HEAD, size=T.SZ_TITLE, color=T.BLUE_DEEP, bold=True)
...
def slide_title_centered(prs):
    ...
    accent.fill.solid()
    accent.fill.fore_color.rgb = rgb(T.BLUE)
...
def slide_section(prs, number="01", title="Section Heading"):
    ...
    _add_text(slide, left, row_top, Inches(1.8), row_h,
              number, font=T.FONT_HEAD, size=Pt(56), color=T.BLUE, bold=True,
...
def slide_two_column(prs, title="Slide title"):
    ...
    frame.line.color.rgb = rgb(T.BLUE)
    ...
    _add_text(slide, text_x, body_top, text_w, Inches(0.5),
              "Caption or claim",
              font=T.FONT_HEAD, size=Pt(22), color=T.BLUE_DEEP, bold=True)
...
def slide_thanks(prs):
    ...
    _add_text(slide, left + Inches(0.35), y,
              ...
              "Thank you",
              font=T.FONT_HEAD, size=Pt(56), color=T.BLUE_DEEP, bold=True)
